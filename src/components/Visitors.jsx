...
const Visitors = () => {
  ...
  const handleEditClick = (visitor) => {
    ...
    setEditFormData({
      ...
      employeeToMeet:
        visitor.employeeToMeet?._id || visitor.employeeToMeet || "",
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (editFormData.visitDateTime) {
        formData.append(
          "visitDateTime",
          new Date(editFormData.visitDateTime).toISOString(),
        );
      }

      const response = await axiosInstance.put(
        `/visitors/${selectedVisitor._id}`,
        formData,
        {
          headers: {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await axiosInstance.put(`/visitors/${selectedVisitor._id}/status`, {
        status: newStatus,
      });
                ...
                {loading ? (
                  ...
                  filteredVisitors.map((visitor) => (
                    <tr key={visitor._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {visitor.employeeToMeet?.name || "N/A"}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-900">
                          {visitor.purpose.substring(0, 30)}...
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadge(visitor.status)}`}
                        >
                          {visitor.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {visitor.visitDateTime
                          ? format(
                              new Date(visitor.visitDateTime),
                              "dd MMM yyyy, hh:mm a",
                            )
                          : "Not scheduled"}
                      </td>
            ...
            <div className="p-6">
              <div className="flex items-center space-x-6 mb-6">
                ...
                <div>
                  ...
                  <span
                    className={`mt-2 px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ${getStatusBadge(selectedVisitor.status)}`}
                  >
                    ...
                    <div>
                      ...
                      <p className="text-gray-900">
                        {selectedVisitor.visitDateTime
                          ? format(
                              new Date(selectedVisitor.visitDateTime),
                              "dd MMM yyyy, hh:mm a",
                            )
                          : "Not scheduled"}
                      </p>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {selectedVisitor.employeeToMeet?.email}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
              ...
              <div className="mt-6 pt-6 border-t border-gray-200 grid grid-cols-2 gap-4 text-sm text-gray-500">
                <div>
                  <p>
                    In Time:{" "}
                    {format(
                      new Date(selectedVisitor.inTime),
                      "dd MMM yyyy, hh:mm a",
                    )}
                  </p>
                </div>
              ...
              {selectedVisitor.meetingStartTime && (
                <div>
                  <p>
                    Meeting Started:{" "}
                    {format(
                      new Date(selectedVisitor.meetingStartTime),
                      "dd MMM yyyy, hh:mm a",
                    )}
                  </p>
                </div>
              ...
              {selectedVisitor.meetingEndTime && (
                <div>
                  <p>
                    Meeting Ended:{" "}
                    {format(
                      new Date(selectedVisitor.meetingEndTime),
                      "dd MMM yyyy, hh:mm a",
                    )}
                  </p>
                </div>
